...
import type { Conversation, NpcResponse, NpcProfile, Debrief } from "./types.js";

const openai = new OpenAI();
const model = process.env.OPENAI_MODEL || "gpt-4o-mini";

export async function generateNpcProfile(scenario: string, language: string): Promise<NpcProfile> {
...
export async function generateNpcOpening(conversation: Conversation): Promise<NpcResponse> {
  const response = await openai.chat.completions.create({
    model,
    response_format: { type: "json_object" },
    messages: [
...
export async function generateNpcResponse(conversation: Conversation): Promise<NpcResponse> {
