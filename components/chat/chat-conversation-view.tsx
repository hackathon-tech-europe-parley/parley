          ...
          {endStatus ? (
            ...
            <form onSubmit={(event) => handleSubmit(event, onSubmit)} className="flex gap-3">
              <input
                ...
                className="flex-1 rounded-xl border border-slate-700 bg-slate-800/50 backdrop-blur-sm px-5 py-3 text-white placeholder-slate-500 transition-all focus:border-blue-500 focus:outline-none focus:ring-2 focus:ring-blue-500/20"
              />
              <button
                type="button"
                onClick={onMuteToggle}
                title={npcMuted ? t("unmute") : t("mute")}
                className="rounded-xl border border-slate-700 bg-slate-800/50 px-4 py-3 text-slate-400 transition-all hover:border-slate-600 hover:bg-slate-700/50"
              >
                {npcMuted ? (
